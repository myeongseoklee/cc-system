...
## 피드백 루프: 실패 시 돌아갈 Phase

검증 실패 시 문제 유형에 따라 적절한 Phase로 돌아가서 수정합니다.

| 실패 유형 | 증상 | 돌아갈 Phase |
|----------|------|-------------|
| **도메인 이해 오류** | 용어 불일치, 협력 시나리오 누락 | Phase 1: 분석 |
| **책임 할당 오류** | 빈약한 도메인, 정보 전문가 위반, 결합도 높음 | Phase 2: 설계 |
| **구현 패턴 오류** | Entity/VO 혼동, Aggregate 경계 오류 | Phase 3: 구현 |
| **테스트 실패** | 단위 테스트 실패, 코드 냄새 남음 | Phase 4: 테스트 |
| **안티패턴 발견** | God Class, Feature Envy 등 | Phase 2 또는 3 |

### 구체적인 판단 기준

```
문제: "로직이 Service에 집중되어 있다"
  → 책임 할당 문제 → Phase 2로

문제: "Entity가 다른 Entity를 직접 수정한다"
  → Aggregate 경계 문제 → Phase 3로

문제: "테스트가 복잡하고 Mock이 많다"
  → 의존성 문제 → Phase 2로

문제: "switch문이 여러 곳에 반복된다"
  → 다형성 미적용 → Phase 2로 (설계) 또는 Phase 4로 (리팩토링)
```
...
← [이전: Phase 4 - 테스트/리팩토링](../04-test-refactor/PHASE.md) | [처음으로: Phase 1 - 분석](../01-analysis/PHASE.md)
