...
```typescript
export class InsertContent {
	async exec(params) {
		// 검증 로직
		if (!params.title) throw new Error('title required');
		if (!params.cpSN) throw new Error('cpSN required');
		if (params.title.length > 100) throw new Error('title too long');

		// 실행 로직
		return await repository.insertContent(params);
	}
}
```
...
export class InsertContent {
	canExecute(params): { valid: boolean; error?: string } {
		if (!params.title) return { valid: false, error: 'title required' };
		if (!params.cpSN) return { valid: false, error: 'cpSN required' };
...
// 테스트: 검증 로직만 독립 테스트
test('title 누락 시 검증 실패', () => {
	const usecase = new InsertContent();
	const result = usecase.canExecute({ cpSN: 1 });
	expect(result).toEqual({ valid: false, error: 'title required' });
});
```
