  ...
  get: AuthInterceptor(
    ...
    async (req, res) => {
      ...
      const dto = SelectListDto.parse({
        ...req.query,
        channelSN: req.local.admin.channelSN,  // admin 정보 주입
    ...
    async (req, res) => {
      try {
        const dto = SelectListAccountBySearchTextDto.parse({
          ...req.query,
          channelSN: req.local.admin.channelSN,
          role: req.local.admin.role,
        });
...
**사용 예:**
- `req.local.admin.channelSN` - Channel 관리자의 channelSN
- `req.local.admin.cpSN` - CP 관리자의 cpSN
- `req.local.admin.role` - MASTER / CHANNEL / CP
- `req.local.admin.payload.refSN` - role에 따른 참조 SN
- `req.local.admin.payload.accountSN` - admin accountSN

## ADMIN_AUTHORITIES 예시

```typescript
ADMIN_AUTHORITIES.ACCOUNT.ALL
ADMIN_AUTHORITIES.STATISTICS.CONTENT
...
- [ ] DTO에 admin 정보 주입 (channelSN, role, refSN 등)
