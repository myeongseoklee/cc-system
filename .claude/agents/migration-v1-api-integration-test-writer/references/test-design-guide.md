...
describe('GET /api/tag', () => {
  ...
  // 예외 케이스
  test('예외: cpSN이 누락된 경우 400 에러', async () => {
    // query에 cpSN 없이 요청
    // 400 응답
  });
...
```typescript
describe('GET /api/tag/[tagSN]', () => {
  test('정상: 태그 상세 조회', async () => {
    // db.mockSelectOne으로 단일 행 반환
  });

  test('예외: 존재하지 않는 tagSN인 경우 404 에러', async () => {
...
describe('POST /api/tag', () => {
  test('정상: 태그 생성', async () => {
    ...
    // 201 응답, data.tagSN 확인
  });

  test('예외: 필수 필드 누락 시 400 에러', async () => {
    // body: { cpSN: 100 } (tag 누락)
  });

  test('예외: 중복된 태그 생성 시 409 에러', async () => {
    // v1 코드가 중복 체크하는 경우
...
```typescript
describe('PUT /api/tag/[tagSN]', () => {
  test('정상: 태그 수정', async () => {
    // db.mockMutation으로 UPDATE 결과
  });

  test('예외: 존재하지 않는 태그 수정 시 404 에러', async () => {
    // db.mockSelectOne(null)
  });
});
```
...
```typescript
describe('DELETE /api/tag/[tagSN]', () => {
  test('정상: 태그 삭제', async () => {
    // db.mockMutation으로 DELETE 결과
  });

  test('예외: 사용 중인 태그 삭제 시 409 에러', async () => {
...
```typescript
// ❌ 나쁜 예
db.mockSelect('SelectTag', [{ tagSN: 1, tag: 'a' }]);
