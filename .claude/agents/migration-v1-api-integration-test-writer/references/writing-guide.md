...
```typescript
// ✅ 올바른 순서 (실행 순서와 동일)
db.mockSelect('SelectTag', [{ tagSN: 1 }]);    // 1번째 호출
db.mockSelect('SelectContent', []);             // 2번째 호출
db.mockMutation('InsertTag', [{ tagSN: 2 }]);  // 3번째 호출

// ❌ 잘못된 순서
db.mockMutation('InsertTag', [{ tagSN: 2 }]);  // 순서 틀림
db.mockSelect('SelectTag', [{ tagSN: 1 }]);
```
...
### ❌ Mock을 describe 레벨에 설정

```typescript
describe('GET /api/tag', () => {
  // ❌ 여기서 설정하면 안됨
  db.mockSelect('SelectTag', [data]);
