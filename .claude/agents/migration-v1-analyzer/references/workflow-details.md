...
```bash
# 도메인 디렉토리 확인
ls -la /Users/myeongseok/Desktop/guru/tc/src_tc_admin_web/src/modules/domain/{domainName}
...
### C. DB 직접 쿼리
```bash
Grep "DB_TC\\." path:service/
Grep "executeQuery" path:service/
```
...
**예시:**
```typescript
if (!cpSN) throw new Error('cpSN 필수');
if (tag.length > 100) throw new Error('태그 길이 초과');
```
...
**예시:**
```typescript
const filtered = allTags.filter(t => t.cpSN === cpSN);
```
...
**SP 추출:**
```bash
Grep "executeQuery\\(['\"]([^'\"]+)" output_mode:content
```
...
{
  "sp": [
    "Webtoon.admin_SelectTagListBySearchText",
    "Webtoon.admin_InsertTag"
  ],
  "tables": ["Tag", "ContentTag"]
}
```
...
**복잡도 기준:**
- **LOW**: 단순 CRUD, SP 1-2개 → 1-2시간
- **MEDIUM**: 트랜잭션, 비즈니스 로직 중간, SP 3-5개 → 3-5시간
- **HIGH**: 복잡한 트랜잭션, 외부 의존성, SP 5개+ → 6-10시간
- **CRITICAL**: Policy 적용, CP 가시성 제어 → 10+시간
