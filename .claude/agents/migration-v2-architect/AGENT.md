...
```typescript
{
  analysisFile: "/tmp/migration/content/v1-analysis.json"  // v1 분석 결과
}
```
...
export class SelectList {
  async exec({ cpSN, pageNo, pageSize, connection }: {
    cpSN: number;
    pageNo: number;
    pageSize: number;
    ...
    return await repository.selectList({ cpSN, pageSize, offset }, connection);
  }
}
```
